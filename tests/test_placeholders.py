...
import pytest
from src.srt_chatgpt_translator.placeholders import PlaceholderManager, load_glossary_file
...
class TestPlaceholderManager:
    ...
    def test_protect_glossary_terms_case_sensitive(self):
        """Test protection of glossary terms (case sensitive)."""
        terms = ["Tokyo", "sensei", "anime"]
        manager = PlaceholderManager(glossary_terms=terms, case_insensitive=False)
        ...
        assert "GLOSSARYTERM_" in protected
        
        # Should have correct replacements
        glossary_replacements = {k: v for k, v in replacements.items() if "GLOSSARYTERM_" in k}
        assert len(glossary_replacements) == 3
        assert "Tokyo" in glossary_replacements.values()
        assert "sensei" in glossary_replacements.values()
        assert "anime" in glossary_replacements.values()
    
    def test_protect_glossary_terms_case_insensitive(self):
        """Test protection of glossary terms (case insensitive)."""
        terms = ["Tokyo", "sensei"]
        manager = PlaceholderManager(glossary_terms=terms, case_insensitive=True)
        ...
        glossary_replacements = {k: v for k, v in replacements.items() if "GLOSSARYTERM_" in k}
        assert len(glossary_replacements) == 2
        assert "TOKYO" in glossary_replacements.values()
        assert "SENSEI" in glossary_replacements.values()
    ...
    def test_restore_text_round_trip(self):
        ...
        terms = ["Tokyo", "sensei"]
        manager = PlaceholderManager(glossary_terms=terms)
    ...
    def test_word_boundaries_respected(self):
        """Test that glossary terms respect word boundaries."""
        terms = ["cat"]
        manager = PlaceholderManager(glossary_terms=terms)
        ...
        glossary_replacements = {k: v for k, v in replacements.items() if "GLOSSARYTERM_" in k}
        assert len(glossary_replacements) == 1
        assert "cat" in glossary_replacements.values()
...
class TestLoadGlossaryFile:
    """Test the load_glossary_file function."""
    
    def test_load_valid_file(self, tmp_path):
        """Test loading a valid glossary file."""
        glossary_file = tmp_path / "glossary.txt"
        glossary_file.write_text("Tokyo\nKyoto\nsensei\n# This is a comment\n\n", encoding="utf-8")
        
        terms = load_glossary_file(str(glossary_file))
    ...
    def test_load_nonexistent_file(self):
        ...
        terms = load_glossary_file("nonexistent.txt")
        assert terms == []
    ...
    def test_load_file_with_only_comments_and_whitespace(self, tmp_path):
        ...
        glossary_file = tmp_path / "comments.txt"
        glossary_file.write_text("# Comment 1\n\n  \n# Comment 2\n\t\n", encoding="utf-8")
