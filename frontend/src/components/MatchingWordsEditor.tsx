...
interface MatchingWordsEditorProps {
  words: MatchingWord[];
  loading: boolean;
  ...
  onImport: (words: MatchingWord[]) => Promise<void>;
  onBulkDelete: (sources: string[]) => Promise<void>;
  description?: string;
}

export default function MatchingWordsEditor({
  words,
  loading,
  onAdd,
  onDelete,
  ...
  const handleAdd = async () => {
    ...
    try {
      await onAdd(source.trim(), target.trim() || source.trim());
      setSource("");
      setTarget("");
  ...
  const saveBulkEdit = async () => {
    try {
      for (const [originalSource, edit] of bulkEdits) {
        await onUpdate(originalSource, edit.source.trim(), edit.target.trim());
      }
      setBulkEdits(new Map());
      setSelected(new Set());
  ...
  const saveEdit = async () => {
    ...
    try {
      await onUpdate(oldSource, editSource.trim(), editTarget.trim());
      setEditIdx(null);
